...
export default function AdminRecordedVideos() {
	...
	const [editing, setEditing] = useState<RecordedVideo | null>(null);
	const [formData, setFormData] = useState({ chapterName: '', youtubeLink: '', category: '' });
	const [errors, setErrors] = useState<{ chapterName?: string; youtubeLink?: string; category?: string }>({});
	...
	const fetchVideos = async () => {
		try {
			const res = await fetch('/api/admin/recorded-videos');
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		// Validate all fields
		const newErrors: { chapterName?: string; youtubeLink?: string; category?: string } = {};
		...
		try {
			let response;
			if (editing) {
				response = await fetch('/api/admin/recorded-videos', {
					method: 'PUT',
			...
			setShowForm(false);
			setEditing(null);
			setFormData({ chapterName: '', youtubeLink: '', category: '' });
			fetchVideos();
		} catch (error) {
	...
	const handleDelete = async (id: number) => {
		...
		try {
			await fetch(`/api/admin/recorded-videos?id=${id}`, { method: 'DELETE' });
			fetchVideos();
		} catch (error) {
			...
			<header className="bg-white border-b">
				<div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
					<div className="flex items-center gap-4">
						...
						<h1 className="text-2xl font-bold">Manage Recorded Videos</h1>
					</div>
					<button
						onClick={() => {
							setShowForm(true);
			...
			<main className="max-w-7xl mx-auto px-4 py-8">
				{showForm && (
					<div className="bg-white rounded-lg border p-6 mb-6">
						<h2 className="text-xl font-semibold mb-4">{editing ? 'Edit Video' : 'Add New Video'}</h2>
							...
							<div>
								...
								<input
									...
									onChange={(e) => {
										...
										if (errors.chapterName) setErrors({ ...errors, chapterName: undefined });
									}}
									className={`w-full rounded-md border px-3 py-2 ${errors.chapterName ? 'border-red-500' : ''}`}
									required
								/>
								{errors.chapterName && <p className="mt-1 text-sm text-red-500">{errors.chapterName}</p>}
							</div>
							<div>
								<label className="block text-sm font-medium mb-1">
					...
					<table className="w-full">
						<thead className="bg-slate-50">
							<tr>
								...
								<th className="px-4 py-3 text-left text-sm font-semibold">Chapter Name</th>
						...
						<tbody>
							{videos.map((video) => (
								<tr key={video.id} className="border-t">
									...
									<td className="px-4 py-3 font-medium">{video.chapterName}</td>
