...
export async function POST(req: NextRequest) {
	try {
		...
		console.log('POST /api/admin/testimonials - Request body:', body);
		...
		const { data: existingTestimonials } = await supabase.from('testimonials').select('id').order('id', { ascending: false }).limit(1);
		const nextId = existingTestimonials && existingTestimonials.length > 0 ? (existingTestimonials[0].id as number) + 1 : 1;
		
		// Remove id from body if present, and use generated ID
		const { id, ...testimonialData } = body;
		const insertData = { ...testimonialData, id: nextId };
		
		console.log('Inserting testimonial with ID:', nextId);
