...
export async function POST(req: NextRequest) {
    try {
        ...
        const rows = (records as any[]).map((r) => ({
            ...
            topic: r.topic || r.title || r.name,
            date: r.date || null,
            time: r.time || null,
            meetingLink: r.meetingLink || r.meeting_link || r.link || r.url,
            description: r.description || null,
        }));

        const { error } = await supabase.from('live_classes').upsert(rows, { onConflict: 'id' });
